...
const Clients = () => {
  ...
  useEffect(() => {
    const verifyToken = async () => {
      try {
        ...
        } = await verifyAndRefreshClientToken();
        if (valid) {
      ...
      const progressPromises = clients.map(async (client) => {
        try {
          const response = await clientApiCall(
            "get",
            `/onboarding/forms-by-recipient/${client.email}`
          );
          ...
          console.error(
            `Error fetching progress for client ${client.email}:`,
            error
          );
          return null;
        }
  ...
  const filteredClients = allClients.filter((client) => {
    const matchesSearch =
      ...
      (client.department?.toLowerCase() || "").includes(
        searchTerm.toLowerCase()
      );
    const matchesStatus = !filters.status || client.status === filters.status;
    const matchesDepartment =
      !filters.department || client.department === filters.department;
    const matchesExperience =
      !filters.experience || client.experience >= parseInt(filters.experience);
    return (
              ...
              {filteredClients.map((client, idx) => (
                ...
                >
                  <td className="px-4 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">
                    {client.clientId || client._id?.slice(-4) || idx + 1}
                  </td>
                  <td className="px-4 py-4 whitespace-nowrap flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="text-xs font-regular text-gray-500 dark:text-gray-400">
                  ...
                  <td className="px-4 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">
                    {client.hireDate
                      ? new Date(client.hireDate).toLocaleDateString()
                      : "-"}
                  </td>
                    ...
                    <div className="flex items-center gap-2">
                      <div className="flex-1 min-w-[120px] max-w-[180px]">
                        <div className="flex items-center justify-between text-xs text-gray-500 dark:text-gray-400 mb-1">
                          <span>
                            {onboardingProgress[client._id]?.status ||
                              "Not Started"}
                          </span>
                          <span>
                            {Math.round(
                        ...
                        <div className="mt-1 text-xs text-gray-500 dark:text-gray-400">
                          {onboardingProgress[client._id]?.completed || 0} of{" "}
                          {onboardingProgress[client._id]?.totalForms || 0}{" "}
                          forms completed
                        </div>
                      </div>
                    </div>
