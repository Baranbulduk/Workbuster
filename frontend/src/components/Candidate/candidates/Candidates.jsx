...
const Candidates = () => {
  ...
  const fetchCandidates = async () => {
    try {
      ...
      const processedCandidates = response.data.map(candidate => ({
        ...
        candidateId: candidate.candidateId || `EM${String(candidate._id).slice(-4)}`,
    ...
    try {
        ...
        try {
          ...
          if (response.data.success) {
            ...
            formsData.forEach(form => {
              const recipient = form.recipients.find(r => r.email === candidate.email);
              if (!recipient) {
                notStarted++;
              } else if (
          ...
          console.error(`Error fetching progress for candidate ${candidate.email}:`, error);
          return null;
        }
      });
      ...
      setOnboardingProgress(prev => {
        ...
        progressResults.forEach((progress, index) => {
          if (progress) {
            newProgress[candidates[index]._id] = progress;
          }
        });
        return newProgress;
      });
  ...
  const allCandidates = [...candidates, ...importedCandidates];
  ...
  const handleExportCSV = () => {
    ...
    const csvData = [...candidates, ...importedCandidates].map(emp => [
      emp.candidateId || emp._id || '',
  ...
  const handleImportCSV = (event) => {
    ...
    if (file) {
      ...
      reader.onload = (e) => {
        ...
        const newCandidates = rows.slice(1).filter(Boolean).map((row, idx) => {
          const values = row.split(',').map(v => v.trim().replace(/"/g, ''));
          return {
            candidateId: values[0] || `imported-${idx}`,
            name: values[1] || '-',
            position: values[2] || '-',
            email: values[3] || '-',
        ...
        setImportedCandidates(newCandidates);
      };
            ...
            <tbody className="bg-white divide-y divide-gray-200 dark:bg-gray-800 dark:divide-gray-700">
              {filteredCandidates.map((candidate, idx) => (
                <tr key={candidate._id} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                  <td className="px-4 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">{candidate.candidateId || candidate._id?.slice(-4) || idx + 1}</td>
                  <td className="px-4 py-4 whitespace-nowrap flex items-center gap-3">
                    <img
                      className="h-10 w-10 rounded-full"
                      src={candidate.avatar || "data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24' fill='%23CBD5E0'%3E%3Cpath d='M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm0 3c1.66 0 3 1.34 3 3s-1.34 3-3 3-3-1.34-3-3 1.34-3 3-3zm0 14.2c-2.5 0-4.71-1.28-6-3.22.03-1.99 4-3.08 6-3.08 1.99 0 5.97 1.09 6 3.08-1.29 1.94-3.5 3.22-6 3.22z'/%3E%3C/svg%3E"}
                      alt={candidate.name || candidate.fullName || 'Candidate'}
                    />
                    <div>
                  ...
                  <td className="px-4 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">{candidate.position || '-'}</td>
                  <td className="px-4 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">{candidate.email || '-'}</td>
                  <td className="px-4 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">
                    {candidate.hireDate ? new Date(candidate.hireDate).toLocaleDateString() : '-'}
                  </td>
                  <td className="px-4 py-4 whitespace-nowrap text-sm">
                    <button
                      className="text-blue-600 hover:underline dark:text-blue-400"
                      onClick={() => navigate(`/candidate/candidates/${candidate._id}${token ? `?token=${token}${email ? `&email=${email}` : ''}` : ''}`)}
                    >
                      View more
                    </button>
                  </td>
                  <td className="px-4 py-4 whitespace-nowrap">
                    ...
                    <div className="flex-1 min-w-[120px] max-w-[180px]">
                        <div className="flex items-center justify-between text-xs text-gray-500 dark:text-gray-400 mb-1">
                          <span>{onboardingProgress[candidate._id]?.status || 'Not Started'}</span>
                          <span>{Math.round(onboardingProgress[candidate._id]?.progress || 0)}%</span>
                        </div>
                        <div className="relative h-2 bg-gray-200 dark:bg-gray-700 rounded-full">
                          <div 
                            className={`absolute left-0 top-0 h-2 rounded-full ${
                              onboardingProgress[candidate._id]?.progress === 100 
                                ? 'bg-green-500' 
                                : onboardingProgress[candidate._id]?.progress > 0 
                                  ? 'bg-blue-500' 
                                  : 'bg-gray-400'
                            }`} 
                            style={{ width: `${onboardingProgress[candidate._id]?.progress || 0}%` }} 
                          />
                        </div>
                        <div className="mt-1 text-xs text-gray-500 dark:text-gray-400">
                          {onboardingProgress[candidate._id]?.completed || 0} of {onboardingProgress[candidate._id]?.totalForms || 0} forms completed
                        </div>
                      </div>
                    </div>
                  </td>
