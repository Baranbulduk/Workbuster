    ...
    if (candidateStr) {
      try {
        ...
        setSettings(prev => ({
          ...
          profile: {
            ...prev.profile,
            name: (candidate.firstName && candidate.lastName) ? `${candidate.firstName} ${candidate.lastName}` : candidate.name || prev.profile.name,
            email: candidate.email || prev.profile.email,
            role: candidate.role || prev.profile.role,
            department: candidate.department || prev.profile.department,
            phone: candidate.phone || prev.profile.phone,
            location: candidate.location || prev.profile.location,
          }
        }));
      } catch (e) {
        // Ignore parse errors
